...
void setup()
{
        ...
        // xTaskCreate(Loop_LoRA_task, "Loop_LoRA_task", 2048, NULL, 1, NULL);
...
void DOWNLINK()
{
        ...
        if (DataRx != "FAILED") {
                ...
                while (ptr < (DataRx.length() - 1)) {
                        if (Data[ptr] == '8' && Data[ptr + 1] == 'e' ) // 00 off //01 on
                        {
                        }
        ...
        if (MySerial.available())
        {
                ...
                if (GET_DATA.data_array[0] == 170 && GET_DATA.data_array[4] > 4 && GET_DATA.data_array[4] != 0 && GET_DATA.data_array[3] == 1)
                {
                        if (currentMillis - DELAY.previousMillis >= DELAY.INTERVAL_DELAY_MONITOR) {
                                ...
                                // seg.LED_monitor(GET_DATA.Sp_val,7);
                                // Serial.print("dimmer  ");
...
void Loop_LED_monitor( void * parameter )
{
        while (1)
        {
                seg.LED_monitor(GET_DATA.Sp_val,8);
                delay(10);
        }
        vTaskDelete( NULL );
}
